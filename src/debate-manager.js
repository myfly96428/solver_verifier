...
class DebateManager {
    ...
    async startDebate(question, cognitoModel, museModel) {
        ...
        try {
            ...
            } else if (!error.message.includes('被用户取消')) {
                this.socket.emit('error', { message: `辩论启动失败: ${error.message}` });
            }
        } finally {
            this.flowLock = false;
            // this.isStopping = false; // **核心修复：移除此处的重置，因为它会过早释放锁**
            console.log('[流程锁] 初始流程结束，锁已释放');
        }
    }
    ...
    async _startDebateFlow(question, cognitoModel, museModel) {
        ...
        this.logger.logDebateFlow('DEBATE_START', { question, cognitoModel, museModel, timestamp: new Date().toISOString() });
    ...
    async integrateSolution() {
        ...
        const context = `**用户原始问题为：**${this.originalQuestion}\n\n**旧版方案 v${this.solutionVersion}为：**\n${this.coreSolution}\n\n**本周期完整的对话记录为：**\n${historyText}\n\n你是Cognito，和Muse刚刚完成了一个辩论周期。请综合这些讨论的所有细节，特别是吸收Muse提出的有效批判和你的修正承诺，用中文将旧版的方案更新为一个保留原有方案**有益**细节与推导的更完善细致、更严谨的新版方案。你的输出应该是完整的、可独立阅读且面向用户原始问题的新版方案全文，而不是修改说明。`;
