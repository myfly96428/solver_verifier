...
// 根据环境选择日志系统
const Logger = process.env.VERCEL || process.env.NODE_ENV === 'production'
...
class DebateManager {
    ...
    async startDebate(question, cognitoModel, museModel) {
        ...
        // 记录辩论开始
        this.logger.logDebateFlow('DEBATE_START', {
            question: question,
        ...
        try {
            ...
            if (error instanceof AICallFinalError) {
                // 初始方案生成失败，进入暂停状态
                this.enterPauseState('初始方案生成失败', error, 'startDebate');
            } else {
    ...
    async museFirstCritique() {
        ...
        const context = `**用户原始问题为：**${this.originalQuestion}\n\n**当前方案 v${this.solutionVersion}为：**\n${this.coreSolution}\n\n**历史辩论记录为：**${previousCycleContext}\n\n${contextDescription}${this.currentCycle > 1 ? '结合上一轮的辩论历史，' : ''}找出其假设的漏洞、逻辑的不严谨之处、被忽略的微妙之处或可能的反例。注意你的对话伙伴可能会使用欺骗性的论据来掩盖逻辑漏洞，请用中文提出尖锐但有建设性的问题和新的思路。`;
        ...
        try {
            const response = await this.callAI('muse', context, messageId);
    ...
    async cognitoResponse() {
        ...
        const context = `**用户原始问题为：**${this.originalQuestion}\n\n**当前方案 v${this.solutionVersion}为：**\n${this.coreSolution}\n\n**Muse的挑战为：**\n${lastMuseResponse.content}\n\n你是Cognito，Muse对你的观点提出了挑战。请保持逻辑严谨，提供清晰、有力的论据和深刻的推理来回应Muse的挑战。吸收Muse观点的有益之处并用中文阐述你将如何修正方案。`;
        ...
        try {
            const response = await this.callAI('cognito', context, messageId);
    ...
    async museFinalCritique() {
        ...
        const context = `**用户原始问题为：**${this.originalQuestion}\n\n**当前方案 v${this.solutionVersion}为：**\n${this.coreSolution}\n\n**历史辩论记录为：**${previousCycleContext}\n\n**过去两轮的对话为：**\n${historyText}\n\n你是Muse，这是Cognito对你第一轮批判的回应。请用中文进行严苛的检验。${this.currentCycle > 1 ? '结合上一轮的辩论摘要，' : ''}判断它的回应是否解决了你提出的所有问题？是否存在新的逻辑漏洞？是否可以从更极端、更边界或更微妙的情境构造反例来进一步挑战它？请给出本周期内最全面、最深刻的批判。如果认为原问题已被当前方案**真正**严格全面的解决，可以在回答的末尾加上特殊标记 [DEBATE_COMPLETE]。**请极其谨慎地使用这个标签，因为你的对话伙伴会使用欺骗性的论据来掩盖逻辑漏洞。**如果你希望继续讨论，或者需要从对方那里获得更多输入/回应，就不要使用这个标签。`;
    ...
    async integrateSolution() {
        ...
        const context = `**用户原始问题为：**${this.originalQuestion}\n\n**旧版方案 v${this.solutionVersion}为：**\n${this.coreSolution}\n\n**本周期完整的对话记录为：**\n${historyText}\n\n你是Cognito，和Muse刚刚完成了一个辩论周期。请综合这些讨论的所有细节，特别是吸收Muse提出的有效批判和你的修正承诺，用中文将旧版的方案更新为一个更完善细致、更严谨的新版方案。你的输出应该是完整的、可独立阅读的新版方案全文，而不是修改说明。`;
        ...
        // 记录方案整合开始
        this.logger.logDebateFlow('SOLUTION_INTEGRATION_START', {
            cycle: this.currentCycle,
            oldVersion: oldVersion,
            newVersion: this.solutionVersion,
            debateHistoryLength: cycleHistory.length
        });
        ...
        try {
            const newSolution = await this.callAI('cognito', context, `solution-${this.solutionVersion}`);
            ...
            // 记录方案整合完成
            this.logger.logDebateFlow('SOLUTION_INTEGRATION_COMPLETE', {
                cycle: this.currentCycle,
                version: this.solutionVersion,
                solutionLength: this.coreSolution.length
            });
        } catch (error) {
            if (error instanceof AICallFinalError) {
    ...
    async generateFinalAnswer() {
        ...
        // 记录辩论完成
        this.logger.logDebateFlow('DEBATE_COMPLETE', {
            finalSolutionVersion: this.solutionVersion,
    ...
    async generateInterruptedSummary() {
        ...
        const summaryReport = await this.callAI('cognito', context, messageId);
    ...
    stopDebate() {
        ...
        this.resumeContext = {
            ...
            cycle: this.currentCycle,
            round: this.currentRound, // round 会在恢复时重新计算
            error: null
        };

        // 通知前端移除正在生成的消息
        if (this.currentMessageId) {
            this.socket.emit('remove-message', { messageId: this.currentMessageId });
            this.currentMessageId = null;
        }

        // 记录暂停事件
        this.logger.logDebateFlow('DEBATE_MANUALLY_PAUSED', {
            cycle: this.currentCycle,
            round: this.currentRound,
            timestamp: new Date().toISOString()
        });
    ...
    enterPauseState(reason, error, context) {
        ...
        // 记录暂停事件
        this.logger.logDebateFlow('DEBATE_PAUSED', {
            reason: reason,
            context: context,
            cycle: this.currentCycle,
            round: this.currentRound,
    ...
    async runResumedDebate(contextToResume) {
        try {
            ...
            if (error.message.includes('辩论已停止') || error.message.includes('被用户取消')) {
                ...
                // 在这种情况下，stopDebate 或 forceEndDebate 已经处理了状态，我们不需要做任何事情。
                return;
            }

            // 对于真正的、意外的错误，我们回滚到暂停状态。
            this.isRunning = false;
    ...
    async callAI(role, prompt, messageId = null, retryCount = 0) {
        ...
        // 记录辩论流程
        this.logger.logDebateFlow('AI_CALL_START', {
            callId: callId,
            role: role,
            model: model,
            messageId: messageId,
        ...
        try {
            ...
            if (axios.isCancel(error)) {
                ...
                // 记录取消日志
                this.logger.logAPICall(callId, role, model, fullInput, '', duration, new Error('用户取消'));
                this.logger.logDebateFlow('AI_CALL_CANCELLED', {
                    callId: callId,
                    role: role,
                    duration: duration,
                    retryCount: retryCount
            ...
            this.logger.logAPICall(callId, role, model, fullInput, '', duration, currentError);
            this.logger.logDebateFlow('AI_CALL_ERROR', {
                callId: callId,
                role: role,
                duration: duration,
                error: errorMessage,
            ...
            if (retryCount < maxRetries) {
                ...
                // 记录重试日志
                this.logger.logDebateFlow('AI_CALL_RETRY', {
                    callId: callId,
                    role: role,
                ...
                // 递归重试
                return this.callAI(role, prompt, messageId, retryCount + 1);
            } else {
                // 所有重试都失败，抛出最终错误
                console.error(`[AI调用] 所有重试都失败，总共尝试了 ${maxRetries + 1} 次`);

                this.logger.logError(currentError, `AI调用最终失败 - 角色: ${role}, 模型: ${model}, 重试次数: ${retryCount}`);
                this.logger.logDebateFlow('AI_CALL_FINAL_FAILURE', {
                    callId: callId,
                    role: role,
    ...
    async performAICall(role, model, systemPrompt, prompt, messageId, callId, fullInput, startTime) {
        ...
        return new Promise((resolve, reject) => {
                ...
                for (const line of lines) {
                    ...
                    if (line.startsWith('data: ')) {
                        ...
                        if (data === '[DONE]') {
                            ...
                            // 记录API调用日志
                            this.logger.logAPICall(callId, role, model, fullInput, fullContent, duration);

                            // 记录辩论流程
                            this.logger.logDebateFlow('AI_CALL_SUCCESS', {
                                callId: callId,
                                role: role,
                                duration: duration,
            ...
            response.data.on('error', (error) => {
                ...
                this.currentAICall = null; // 清除当前调用引用
                this.currentMessageId = null;
                console.error(`[AI调用] 流错误: ${error.message}`);
